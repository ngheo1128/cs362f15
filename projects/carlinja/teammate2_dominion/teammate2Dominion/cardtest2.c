...
Tests if smithy card is removed from hand
*/
...
int main(){
	...
	for(i=0; i<27; i++) {
		...
		for(j=0; j<MAX_PLAYERS; j++){
			...
			//test if smithy card was discarded
			//ignore test if decks are made up of smithy cards
			if(i != smithy){
				//smithy card removed from hand
				if(state.hand[j][0] != smithy){
					printf("PASS - Smithy card removed from hand for player %d\n", j+1);
					passCount++;
				}
				else{
					printf("FAIL - Smithy card was not removed from hand for player %d\n", j+1);
					failCount++;
				}
				testCount++;
			}
