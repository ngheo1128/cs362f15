...
int main ()  {
	...
	((char*)&G)[i] = floor(Random() * 256);
      ...
      G.deckCount[p] = floor(Random() * (MAX_DECK))/20;
      G.discardCount[p] = floor(Random() * (MAX_DECK))/20;
      G.handCount[p] = floor(Random() % MAX_HAND)/10;
     ...
     // printf("handCount:%d\n",G.handCount[p]);
      ...
      cardEffect_smithy(0, currentPlayer,&g);
