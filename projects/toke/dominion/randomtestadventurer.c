...
// Random Test Objective: Tests the executeAdventurerCard method
...
int main () {
  ...
  printf ("Random Tests for executeAdventurerCard().\n");
  ...
  for (n = 0; n < numRandomTests; n++) {
    ...
    SetCoinSupply(&G);
    checkAdventureCard(player, &G);
  }

  return 0;
}

int checkAdventureCard(int player, struct gameState *state) {
  // Test Checklist: 
  ...
  int preTreasureCount = countTreasuresInHand(player, state);
  ...
  int postTreasureCount = countTreasuresInHand(player, state);
  ...
  printf("Test - Expected amount of treasures added to hand: %d, Actual amount: %d -- ", preTreasureCount + 2, postTreasureCount);
  testPassed = postTreasureCount == preTreasureCount + 2;
  checkFailure(testPassed);
  overallTestsPassed = overallTestsPassed ? (testPassed ? 1 : 0) : 0;
  assert (testPassed);
