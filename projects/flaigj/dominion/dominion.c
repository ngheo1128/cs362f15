...
void adventurerCard(int currentPlayer, struct gameState *state, int drawntreasure, int z, int temphand[MAX_HAND], int cardDrawn, int handPos)
{
...
void tributeCard(int currentPlayer, int nextPlayer, struct gameState *state, int tributeRevealedCards[2])
{
   int i;
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
   ...
   {
      case adventurer:
	 adventurerCard(currentPlayer, state, drawntreasure, z, temphand, cardDrawn, handPos);
	 return 0;
	 ...
	 for (i = 0; i < state->numPlayers; i++)
	 {
	    if (i != currentPlayer)
	    {
	       state->discard[i][state->discardCount[i]++] = state->deck[i][state->deckCount[i]-1]; 	// discard top card from deck
     	       //state->deckCount[i]--;		// decrement deck count
	       //state->discardCount[i]++;	// increment discard count
	       state->deck[i][state->deckCount[i]-1] = curse;//Top card now a curse
	       state->supplyCount[curse]--;	// decrement curse card count
	    }
	 }   
	 //discardCard(handPos, currentPlayer, state, 0);
