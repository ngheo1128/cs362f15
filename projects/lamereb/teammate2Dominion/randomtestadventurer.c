...
int printRelevantGameState(struct gameState* gs, int player) {
  ...
  */
  ...
  for (i = 0; i < gs->handCount[player]; i++) {
    printf("--hand[%d][%d] = %d", player, i, gs->hand[player][i]);
    if (gs->hand[player][i] == 4) printf(" COPPER");
    if (gs->hand[player][i] == 5) printf(" SILVER");
    if (gs->hand[player][i] == 6) printf(" GOLD");
    printf("\n");
  }
  printf("deckCount[%d] = %d\n", player, gs->deckCount[player]);
  for (i = 0; i < gs->deckCount[player]; i++) {
    printf("--deck[%d][%d] = %d", player, i, gs->deck[player][i]);
    if (gs->deck[player][i] == 4) printf(" COPPER");
    if (gs->deck[player][i] == 5) printf(" SILVER");
    if (gs->deck[player][i] == 6) printf(" GOLD");
    printf("\n");

  }
  printf("playedCardCount = %d\n", gs->playedCardCount);
  for (i = 0; i < gs->playedCardCount; i++) {
    printf("playedCards[%d] = %d", i, gs->playedCards[i]);
    if (gs->playedCards[i] == 4) printf(" COPPER");
    if (gs->playedCards[i] == 5) printf(" SILVER");
    if (gs->playedCards[i] == 6) printf(" GOLD");
    printf("\n");

  }
  printf("discardCount[%d] = %d\n", player, gs->discardCount[player]);
  for (i = 0; i < gs->discardCount[player]; i++) {
    printf("--discard[%d][%d] = %d", player, i, gs->discard[player][i]);
    if (gs->discard[player][i] == 4) printf(" COPPER");
    if (gs->discard[player][i] == 5) printf(" SILVER");
    if (gs->discard[player][i] == 6) printf(" GOLD");
    printf("\n");
...
int main() {
  ...
  printf(" -- ADVENTURER CARD TESTED %d TIMES  WITH %d ERRORS\n", testRuns, errorCount);
