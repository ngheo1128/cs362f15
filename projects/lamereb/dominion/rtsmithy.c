...
// function to randomly test the smithycard
int randomTestSmithy(int playerCount, int playerTested) {
  ...
  // randomize gameState vars relevant to the smithy card
  randomizeGameState(&g, playerTested);

  // make sure g.hand[playerTested][0] is smithy card
  // NOTE: can't just do below for players who aren't 'playing'
  if (playerTested == 0) {
    g.hand[playerTested][0] = smithy;
  }
  else {
    gainCard(smithy, &g, 2, playerTested);
  }
...
#if VERBOSE
  printf(" -- RANDOMIZED GAME STATE BEFORE PLAYING SMITHY CARD -- \n");
  ...
  */ 
  ...
  // assert that played smithy card has gone on top of playedCard or discard decks
  if ((g.hand[playerTested][0] == smithy) && (g.playedCards[0] != smithy)) {
    if (g.discard[playerTested][0] != smithy) {
      errorCount++;
      printf("ERROR: expected smithy card atop played or discard piles, but it isn't\n");
    }
  }
...
int main() {
  ...
  for (i = 0; i < loops; i++) {
    errorCount += randomTestSmithy(2, 0);
    testRuns++;
    errorCount += randomTestSmithy(3, 0);
    testRuns++;
    errorCount += randomTestSmithy(4, 0);
    testRuns++;
  }
