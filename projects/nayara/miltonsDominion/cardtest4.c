...
int main() {
    ...
    for (p = 0; p < numPlayer; p++)
    {
        ...
        for (handCount = 1; handCount <= maxHandCount; handCount++)
        {
            ...
            assert(G.coins == 5);
#if (NOISY_TEST == 1)
            printf("Testing for council card. Expected %d, received %d\n",testCard, G.hand[p][0]);
#endif
            ...
            assert(G.coins == 5);
#if (NOISY_TEST == 1)
            printf("Testing for replaced council card. Expected %d, received %d\n",testCard, G.hand[p][0]);
#endif
            assert(G.hand[p][0] != testCard);
#if (NOISY_TEST == 1)
            printf("Hand count. Expected %d, received %d\n",handCount, G.handCount[p]);
            ...
            if (p ==0 )
            {
                //assert(G.handCount[p] == handCount);
                //assert(G.handCount[1] == 0);
                if (G.handCount[p] != handCount+3 || G.handCount[1] != 1)
                {
                    
                    printf("FAILURE: Hands have too many or too few cards.\n");
                    printf("%d, %d", G.handCount[p], G.handCount[1]);
                }
            }
            else
            {
                //assert(G.handCount[p] == handCount);
                //assert(G.handCount[0] == 0);
                if (G.handCount[p] != handCount+3   || G.handCount[0] != 1)
                {
                    printf("FAILURE: Hands have too many or too few cards.\n");
                }
