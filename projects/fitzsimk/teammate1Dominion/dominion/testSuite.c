...
void testDeckShuffle(struct gameState* G){
    ...
    printf("results: \n");
    // playAdventurer(G);           //takes player, game ptr, and hand position of adventurer card
...
void testEffectsOtherPlayers(struct gameState* game){
    ...
    int result = cardEffect(cutpurse, -1, -1, -1, game, 0, 0);
//    int result=play_cutpurse(0,game,0);
  ...
  void testZeroTreasureMap(struct gameState *G){
     ...
     int player=G->whoseTurn;
       ...
       int result = cardEffect(treasure_map, -1, -1, -1, G, 0, 0);
     ...
     //   printf("TURN %d\n", m);
       ...
       printf("PLAYER %d\n", player);
        ...
        G->hand[player][3]= 26;
          ...
          int result = cardEffect(treasure_map, -1, -1, -1, G, 1, 0);  
...
int main(){
 ...
 printf ("\t\t\t\t\tFINISHED tests for function endTurn...\n\n");
  ...
  //                      CARD TEST 1                         
    ...
    sea_hag, tribute, smithy};
   printf ("\t\t\t\t\tStarting Card Test 1for function smithyCard...\n\n");
   initializeGame(2, ct1k, 1000, &G);
   testDrawThreeCards(&G);
   testSmithyIsDiscarded(&G);
   printf ("\t\t\t\t\tFINISHED Unit Tests for function smithyCard...\n\n"); 
     /*                            CARD TEST 2               */          
