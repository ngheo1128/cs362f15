...
int testAdventurerCard(struct gameState *state)
{
    ...
    if(state->handCount[currentPlayer] == origState->handCount[currentPlayer]+1)
    {
        ...
        printf("playAdventureCard: PASS correct number of cards added to hand\n");
    ...
    {
        printf("playAdventureCard: FAIL incorrect number of cards added to hand\n");
    }
    ...
    // See if the number of discarded cards is correct. Should be at least
    // 1 new discarded card, possibly others depending on what's in tmp
    // hand.
    /*
    ...
    if(diffDiscardCount + 1 <= diffDeckCount)
    {
