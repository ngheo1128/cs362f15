...
int main(int argc, char *argv[]){
    ...
    activateSmithy(&state, 0, 0);

    printf("activateSmithy(): %s handCount properly incremented\n", 
        (state.handCount[0] == 3 &&
    ...
    printf("activateSmithy(): %s correct cards, order in correct user's hand\n", 
        (state.hand[0][0] == province &&
        state.hand[0][1] == duchy &&
        state.hand[0][2] == gold) ? "PASS" : "FAIL");

    /* clear verified changes made by activateSmithy() */
    ...
    printf("activateSmithy(): %s changes to correct player only\n", 
