...
int main(int argc, char *argv[]){
    ...
    cardEffect_smithy(&state, 0, 0);

    printf("cardEffect_smithy(): %s handCount properly incremented\n", 
        (state.handCount[0] == 3 &&
    ...
    printf("cardEffect_smithy(): %s correct cards, order in correct user's hand\n", 
        (state.hand[0][0] == province &&
        state.hand[0][1] == duchy &&
        state.hand[0][2] == gold) ? "PASS" : "FAIL");

    /* clear verified changes made by cardEffect_smithy() */
    ...
    printf("cardEffect_smithy(): %s changes to correct player only\n", 
