...
//        }
        ...
        for (p = 0; p < curPlayers; ++p) {
            ...
            state.whoseTurn = p;
            int card = adventurer;
            int temphand[500];// moved above the if statement
            adventurerCard (0, &state, state.handCount[p], card, temphand, 0);

            //compare # of treasure cards


            for (x = 0; x < state.handCount[p]; ++x) {
                if (state.hand[p][x] == 4 || state.hand[p][x] == 5 || state.hand[p] == 6) {
