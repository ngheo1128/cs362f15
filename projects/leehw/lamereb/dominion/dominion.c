...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  {
    case adventurer:
      return (playAdventureCard(state)); 

    case council_room:
      return (playCouncilRoomCard(state, handPos));
    ...
    case remodel:
      return (playRemodelCard(state, handPos, choice1, choice2));
...
int playAdventureCard(struct gameState *state) {
  int currentPlayer = whoseTurn(state);
  int nextPlayer = currentPlayer + 1;
  ...
  printf("after discard while loop\n");
  return 0;
}
...
int playCouncilRoomCard(struct gameState *state, int handPos) {
  int i;
  int currentPlayer = whoseTurn(state);
...
int playRemodelCard(struct gameState *state, int handPos, int choice1, int choice2) {
  int currentPlayer = whoseTurn(state);
