...
int checkCouncil(int p, struct gameState post, int handPos, struct failures *failure, int currentPlayer) {
  ...
  playCouncilRoomCard(&post, handPos);
  ...
  if (post.hand[currentPlayer][pre.handCount[currentPlayer] + 3] == council_room){
    councilCount++;
  }
  ...
  if (preCouncil - 1 + councilCount != postCouncil){
