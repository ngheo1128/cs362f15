...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  switch( card ) 
    {
    case adventurer:
      adventurerAction(currentPlayer, state, temphand, handPos);
      return 0;
    case council_room:
      councilAction(currentPlayer, state, handPos);
      return 0;
    case feast:
    ...
    case baron:
      baronAction(currentPlayer, state, choice1, handPos);
      return 0;
    ...
    case treasure_map:
      ...
      //no second treasure_map found in hand
      //BUG FIX: SL 11.21 - discard treasurecard if 2 not in hand
  ...
  if (trashFlag < 1)
    {
      ...
      state->playedCardCount++;
      // add card to discard pile - SL BUG FIX 11.20.15 - Code inspired by Erik Ratclier
...
int adventurerAction(int player, struct gameState *state, int temphand[], int handPos){
  ...
  while(drawntreasure<2){
    ...
    //BUG FIX: SL 11/21/15 - exit condition for treasure finding loop
    if (result != -1){
      drawCard(player, state);
...
int baronAction(int player, struct gameState *state, int choice1, int handPos){
  ...
  if (choice1 > 0){//Boolean true or going to discard an estate
    ...
    while(card_not_discarded){
      if(state->hand[player][p] == estate){//Found an estate card!
        state->coins += 4;//Add 4 coins to the amount of coins
        state->discard[player][state->discardCount[player]] = state->hand[player][p];
        state->discardCount[player]++;
        for (;p < state->handCount[player]; p++){
          state->hand[player][p] = state->hand[player][p+1];
        }
        state->hand[player][state->handCount[player]] = -1;
        state->handCount[player]--;
        card_not_discarded = 0;//Exit the loop
      }else if (p > state->handCount[player]){
        if(DEBUG) {
  ...
  //player decides not to discard an estate card
  }else{
    //if estate card pile is not empty, draw one
  	if (supplyCount(estate, state) > 0){
	    gainCard(estate, state, 0, player);//Gain an estate
	    state->supplyCount[estate]--;//Decrement Estates
    }
    //else if it is empty, 
	  if (supplyCount(estate, state) == 0){
