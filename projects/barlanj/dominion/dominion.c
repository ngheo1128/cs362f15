...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
    ...
    {
    case adventurer:
        cardEffect_adventurer(drawntreasure, state, currentPlayer, cardDrawn, temphand);
        return 0;

    case council_room:
        cardEffect_councilRoom(currentPlayer, state, handPos);
        return 0;
    ...
    case smithy:
        cardEffect_smithy(currentPlayer, state, handPos);
        return 0;
    ...
    case steward:
        cardEffect_steward(choice1, choice2, choice3, currentPlayer, state, handPos);
        return 0;
    ...
    case embargo:
        if (cardEffect_embargo(state, choice1, handPos, currentPlayer) == -1)
            return -1;
        return 0;
...
void cardEffect_smithy(int currentPlayer, struct gameState* state, int handPos)
{
        //+3 Cards
        int i;
        for (i = 0; i < 3; i++)
...
void cardEffect_councilRoom(int currentPlayer, struct gameState* state, int handPos)
{
        int i;
        //+4 Cards
...
int cardEffect_embargo(struct gameState* state, int choice1, int handPos, int currentPlayer)
{
        //+2 Coins
        state->coins = state->coins + 2;
...
void cardEffect_steward(int choice1, int choice2, int choice3, int currentPlayer, struct gameState* state, int handPos)
{
        if (choice1 == 1)
        {
            //+2 cards
