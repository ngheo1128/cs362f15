...
int adventurer_play(struct gameState *state, int currentPlayer, int handPos) {
    int z =0;
    int drawntreasure =0;
    int cardDrawn;
...
int village_play (struct gameState *state, int currentPlayer, int handPos) {
    int numActions = 0;
    //+1 Card
    drawCard(currentPlayer, state);
...
int great_hall_play (struct gameState *state, int currentPlayer, int handPos) {
    //+1 Card
    drawCard(currentPlayer, state);
...
int cardEffect(int card, int choice1, int choice2, int choice3, struct gameState *state, int handPos, int *bonus)
{
  ...
  if (nextPlayer > (state->numPlayers - 1)){
    ...
    if (card == adventurer) {
        adventurer_play(state, currentPlayer, handPos);
    }
    if (card == council_room) {
        council_room_play(state, currentPlayer, handPos);
    }
    if (card == smithy) {
        smithy_play(state, currentPlayer, handPos);
    }
    if (card == village) {
        village_play(state, currentPlayer, handPos);
    }
    if (card == great_hall) {
        great_hall_play(state, currentPlayer, handPos);
    }
